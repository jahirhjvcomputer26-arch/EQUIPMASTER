...
export default function GarantiasMantenimiento() {
  ...
  const cargar = () => Promise.all([api.getGarantias(), api.getMantenimientos()]).then(([g, m]) => { setGarantias(g); setMantenimientos(m); }).catch(() => {});
  useEffect(() => { cargar(); }, []);
